  ...
  for (let i = 0; i <= MAX_REDIRECTS; i++) {
    ...
    try {
      const res = await fetch(current, {
        ...
        headers: {
          "user-agent": "PerceviaAI/1.0 accessibility scanner",
...
function unavailablePageIssue(errorMessage: string): NormalizedIssue {
  return issue({
    ...
    description:
      "Percevia could not retrieve this page during the scan, so automated accessibility checks could not run for it.",
