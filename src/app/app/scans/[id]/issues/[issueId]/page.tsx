...
import {
  ...
  getScanJob,
  getScanPage,
  getVisualEvidenceForIssue,
} from "@/lib/data/firestore";
import type { VisualEvidence } from "@/lib/data/types";
...
export default async function IssueDetailPage({
  ...
  const [scan, issue] = await Promise.all([
  ...
  const [page, evidence] = await Promise.all([
    issue.scanPageId ? getScanPage(ctx.workspace.id, id, issue.scanPageId) : null,
    getVisualEvidenceForIssue(ctx.workspace.id, issueId),
  ]);

  const tags = issue.wcagTagsJson ?? [];
  const level = tags.includes("wcag2aaa") ? "AAA" : tags.includes("wcag2aa") ? "AA" : "A";
  const criterion = tags.find((t) => /^wcag\d/.test(t)) ?? "—";
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 mb-6">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            ...
            {issue.falsePositive && (
              <Badge tone="neutral" size="sm">False positive</Badge>
            )}
          </div>
          ...
          {issue.htmlSnippet && (
            <Card>
              ...
              <CardContent>
                ...
                {issue.targetJson && issue.targetJson.length > 0 && (
                  <p className="text-[11px] text-ink-500 mt-2 font-mono">
                    Selector: {issue.targetJson.join(" >> ")}
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-4 pt-4 border-t border-line space-y-3 text-xs">
                ...
                {issue.contextsJson && issue.contextsJson.length > 0 && (
                  <KeyVal label="Seen in">
                    {issue.contextsJson
                      .map((c) => `${c.viewport}/${c.state}`)
                      .join(", ")}
                  </KeyVal>
                )}
              </div>
...
function evidenceReason(reason: string): string {
  const labels: Record<string, string> = {
    ...
    screenshot_too_large_for_firestore: "The screenshot exceeded the storage size limit.",
  };
