...
const EMAIL_STORAGE_KEY = "accessops_email_for_sign_in";
  ...
  async function sendEmailLink(e: React.FormEvent) {
    ...
    try {
      await sendSignInLinkToEmail(firebaseClientAuth(), email.trim(), {
        url: `${window.location.origin}/auth/sign-in?callbackUrl=${encodeURIComponent(callbackUrl)}`,
    ...
    <div className="min-h-screen bg-canvas-2 flex flex-col">
      ...
      >
        ...
        <p className="text-xs text-ink-500 mt-6 leading-relaxed">
          ...
          acknowledge that AccessOps AI does not guarantee legal compliance.
        </p>
      </main>
    </div>
  );
