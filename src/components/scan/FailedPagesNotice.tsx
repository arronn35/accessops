import { AlertTriangle } from "lucide-react";
import type { PageJob } from "@/lib/data/types";

export function FailedPagesNotice({
  jobs,
  failedUrls = [],
  pagesScanned,
}: {
  jobs: PageJob[];
  failedUrls?: string[];
  pagesScanned: number;
}) {
  const failures = new Map<
    string,
    { key: string; url: string; detail: string | null }
  >();
  for (const job of jobs) {
    failures.set(job.url, {
      key: job.id,
      url: job.url,
      detail: job.error ?? job.errorCode ?? "Page scan failed",
    });
  }
  for (const url of failedUrls) {
    if (!failures.has(url)) {
      failures.set(url, { key: url, url, detail: null });
    }
  }
  const rows = [...failures.values()];
  if (rows.length === 0) return null;

  return (
    <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
      <div className="flex gap-2">
        <AlertTriangle className="size-4 shrink-0 mt-0.5" aria-hidden />
        <div className="min-w-0">
          <p className="font-medium">
            {rows.length} page{rows.length === 1 ? "" : "s"} could not be scanned
          </p>
          <p className="mt-1 text-amber-900">
            The score and findings below use the {pagesScanned} page
            {pagesScanned === 1 ? "" : "s"} that completed successfully.
          </p>
          <ul className="mt-2 space-y-1 font-mono text-xs">
            {rows.map((row) => (
              <li key={row.key} className="break-all">
                {row.url}
                {row.detail ? ` - ${row.detail}` : ""}
              </li>
            ))}
          </ul>
        </div>
      </div>
    </div>
  );
}
